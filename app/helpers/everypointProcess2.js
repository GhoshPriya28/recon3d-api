...
const {EVP_BASE_URL_NEW,EVP_API_VERSION,EVP_API_KEY,EVP_API_SECRET,EVP_API_INSTANCE} = process.env;
const FINAL_BASE_URL = EVP_BASE_URL_NEW+'v2/'+EVP_API_INSTANCE
...
console.log('Final EV Url',FINAL_BASE_URL);
...
{
	console.log('End point + req body',apiEndpoint , requestBody)
...
exports.makeGetRequestNew = async function (apiEndpoint,requestBody = null,header = null)
{
	...
	//console.log(apiEndpoint , requestBody)
	return await axios.get(apiEndpoint,requestBody).then(function (response) {
    	//console.log('Get New Response',response);
    	return response;
  	}).catch(function (error) {
    	return error;
  	});
...
exports.makeRequestforChrone = async function (apiEndpoint,requestBody = null)
{
