					...
					if(user)
					{
					   ...
					   var generatedPassword = utility.generatePassword();
						...
						UserModel.update(updateData, {where:{email : req.body.email}}).then(data => {
                           if(!req.body.linkUrl)
							 {
								var link = `${BASE_URL}/reset-password?username=${user.email}&password=${generatedPassword}`;
							 }
							 else
							 {
								var link = `${linkUrl}/reset-password?username=${user.email}&password=${generatedPassword}`;
							 }
