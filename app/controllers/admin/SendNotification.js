...
exports.UserlistNotification = [
  (req, res) => {
    ...
    UserModel.findAll().then(data => {
      res.render('userlistNotification', { posts: req.session.name, data: data ,type:req.session.type});
    });
  }
]
...
exports.send_notificationWithMultiDevice =
  [
    async (req, res) => {
      if (req.session.name) {
        var userId = { id: req.body.user_ids };
  ...
  (req, res) => {
    NotificationDetailModel.findAll().then(data => {
      res.render('notification',{ posts:req.session.name,data:data,type:req.session.type});
    });
  }
]
              ...
              if (user) {
                ...
                user.forEach(function (value, index) {
                  console.log(value);
                    ...
                    UserNotificationModel.create({ user_id: value.id, user_device_id: value.user_firebase_id?value.user_firebase_id:null, notification_id: notifications.id })
                    });
                const returnData =  notificationUtility.sentNotificationMultiple(firebaseIds, title, data,notifications.id);
...
exports.notificatidetail=[
  (req, res) => {
    ...
    var user_sql = "select u.first_name,u.last_name,n.id,n.title,n.notification,n.log from users_notifications d JOIN app_users u ON u.id=d.user_id JOIN notification_details n ON n.id=d.notification_id WHERE n.id="+notification_id+" ";
    UserNotificationModel.sequelize.query(user_sql).then((result) => {
      ...
      // console.log(UserListAr);
       res.render('notificationdetail',{ posts:req.session.name,data:UserListAr,type:req.session.type});
    });
