	    ...
	    else
	    {
	    	...
	    	FileTaskModel.findOrCreate({where:{initiate_id: initiateId},defaults: initiateData}).then(initiateDetails => {
	    		const [ object, created ] = initiateDetails;
					...
					{
						const fileExt = object.file_ext;
						const mergeFileName = initiateId+"-merge-file."+fileExt;
						...
						fs.readdir('./uploads/3d-requests/'+initiateId, (err, files) => {
							...
							if(body.totalChunks == files.length)
							{
								...
								fsMerge(inputPathList, outputPath).then(response => {
									...
									uploadFileToS3(fileContent,initiateId,mergeFileName,mergeFileName).then(s3FileUrl => {
...
const uploadMultipleFileToS3 = async (files, folderName) =>
{	
	var responseData = [];
	return await Promise.all(
		files.map(async (item) => {
			let bucketPath = encodeURIComponent(folderName)+'/'+item.originalname;
			const fileContent = fs.readFileSync(item.path)
			var params = {
				Bucket: bucketName,
				Key: bucketPath,
				Body: fileContent,
